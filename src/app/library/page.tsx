...
function ResourceCard({ resource }: { resource: LibraryResource }) {
  return (
    <a
      ...
      className="group relative flex flex-col gap-3 rounded-xl border border-gray-900 dark:border-white bg-white dark:bg-[#1a1a1a] p-5 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_8px_24px_rgba(0,0,0,0.06)]"
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-gray-900 dark:text-white font-mono mt-1">
            {resource.provider}
          </p>
        </div>
        <ArrowUpRight
          size={16}
          className="shrink-0 text-gray-900 dark:text-white transition-all duration-200 group-hover:translate-x-0.5 group-hover:-translate-y-0.5"
        />
      </div>
      ...
      <section className="hidden md:block relative min-h-screen bg-white dark:bg-[#111]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-20">
          ...
          {visibleResources.length === 0 && (
            <p className="py-16 text-center text-sm text-gray-900 dark:text-white font-mono">
